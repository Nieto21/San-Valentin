...
export const useChangeElements = () => {
    ...
    const options = ['Segura?', 'Segurisima?', 'Estas completamente segura?', 'No te arrepentiras?', 'Pero si estas segura?', 'No hay vuelta atras', 'No hay devoluciones', 'No hay garantias', 'No hay reembolsos', 'No hay cambios', 'No hay nada', 'No hay', 'Que no hay',"O no", "O nada?","O nadota?","andele papa","andele","si o queso?"]
    ...
    const handleButtonYes = () => {
        ...
        setTextP('Gracias por aceptar princesa hermosa, te amo mucho lindota <3')
        confetti()
    }
